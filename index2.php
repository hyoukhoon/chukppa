...
$today=date("Y-m-d");
$k=0;
$query="select g.hometeam, g.awayteam, g.gamedate, g.gametime, g.country, g.league, k.player from xc_games2 g
join koreanteam k on (g.hometeam=k.teamname or g.awayteam=k.teamname)
where g.gamedate>='".$today."' and g.gubun=1 and g.status!='취소' order by rand() limit 6";
...
if(isMobile()){
	...
	$query="select g.hometeam, g.awayteam, g.gamedate, g.gametime, g.country, g.league, k.player from xc_games2 g
	join koreanteam k on (g.hometeam=k.teamname or g.awayteam=k.teamname)
	where g.gamedate>='".$today."' and g.gubun=1 and g.status!='취소' order by g.gamedate asc, g.gametime asc";
	$result = $mysqli->query($query) or die("query error => ".$mysqli->error);
	while($rs = $result->fetch_object()){
		$rsp[]=$rs;
	}
