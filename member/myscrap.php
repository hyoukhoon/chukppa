...
<main class="container">
  ...
  <div class="my-3 p-3 bg-body rounded shadow-sm">
    <h6 class="border-bottom pb-2 mb-0"><span class="material-symbols-outlined" style="vertical-align: text-bottom;">all_inbox</span>스크랩한 짤</h6>

<?php
		$sql = "select * from xc_cboard c
		join xc_scrap s on c.num=s.bid
		where s.userid='".$_SESSION['loginValue']['SEMAIL']."'";
        $sql .= " and c.isdisp=1 and s.isuse=1";
        if(!empty($sword)){
            $search_where = " and (c.subject like '%".$sword."%' or c.content like '%".$sword."%')";
        }
        $sql .= $search_where;
        $order = " order by s.num desc";
        if($page < 1) $page = 1;
...
//	exit;
		...
		join xc_scrap s on c.num=s.bid
		where s.userid='".$_SESSION['loginValue']['SEMAIL']."'";
        $sqlcnt .= " and c.isdisp=1 and s.isuse=1";
        $sqlcnt .= $search_where;
...
?>
		...
		<span class="d-block"><?php echo $r->name;?> / <?php echo $r->regdate;?></span>
		</div>
    </div></a>
<?php
	}
