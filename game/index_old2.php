...
$query="select * from xc_games where gamedate>='".$today."' and (hometeam='토트넘' or awayteam='토트넘' or hometeam='토트넘 (Eng)' or awayteam='토트넘 (Eng)') order by gamedate asc, gametime asc";
$result = $mysqli->query($query) or die("query error => ".$mysqli->error);
while($rs = $result->fetch_object()){
	$rsc1[]=$rs;
}

$query="select * from xc_games where gamedate>='".$today."' and (hometeam='울버햄튼' or awayteam='울버햄튼' or hometeam='울버햄튼 (Eng)' or awayteam='울버햄튼 (Eng)') order by gamedate asc, gametime asc";
$result = $mysqli->query($query) or die("query error => ".$mysqli->error);
while($rs = $result->fetch_object()){
	$rsc2[]=$rs;
}

$query="select * from xc_games where gamedate>='".$today."' and (hometeam='바이에른 뮌헨' or awayteam='바이에른 뮌헨' or hometeam='바이에른 뮌헨 (Ger)' or awayteam='바이에른 뮌헨 (Ger)') order by gamedate asc, gametime asc";
$result = $mysqli->query($query) or die("query error => ".$mysqli->error);
while($rs = $result->fetch_object()){
	$rsc3[]=$rs;
}

$query="select * from xc_games where gamedate>='".$today."' and (hometeam='파리생제르망' or awayteam='파리생제르망' or hometeam='파리생제르망 (Fra)' or awayteam='파리생제르망 (Fra)') order by gamedate asc, gametime asc";
$result = $mysqli->query($query) or die("query error => ".$mysqli->error);
while($rs = $result->fetch_object()){
	$rsc4[]=$rs;
}

$query="select * from xc_games where gamedate>='".$today."' and (hometeam='노팅엄' or awayteam='노팅엄' or hometeam='노팅엄 (Eng)' or awayteam='노팅엄 (Eng)') order by gamedate asc, gametime asc";
$result = $mysqli->query($query) or die("query error => ".$mysqli->error);
while($rs = $result->fetch_object()){
	$rsc5[]=$rs;
}

$query="select * from xc_games where gamedate>='".$today."' and (hometeam='미튈란' or awayteam='미튈란' or hometeam='미튈란 (Den)' or awayteam='미튈란 (Den)') order by gamedate asc, gametime asc";
$result = $mysqli->query($query) or die("query error => ".$mysqli->error);
while($rs = $result->fetch_object()){
	$rsc6[]=$rs;
}

$query="select * from xc_games where gamedate>='".$today."' and (hometeam='올림피아코스' or awayteam='올림피아코스' or hometeam='올림피아코스 (Gre)' or awayteam='올림피아코스 (Gre)') order by gamedate asc, gametime asc";
$result = $mysqli->query($query) or die("query error => ".$mysqli->error);
while($rs = $result->fetch_object()){
	$rsc7[]=$rs;
}

$query="select * from xc_games where gamedate>='".$today."' and (hometeam='셀틱' or awayteam='셀틱' or hometeam='셀틱 (Sco)' or awayteam='셀틱 (Sco)') order by gamedate asc, gametime asc";
$result = $mysqli->query($query) or die("query error => ".$mysqli->error);
while($rs = $result->fetch_object()){
	$rsc8[]=$rs;
}

$query="select * from xc_games where gamedate>='".$today."' and (hometeam='헨트' or awayteam='헨트' or hometeam='헨트 (Bel)' or awayteam='헨트 (Bel)') order by gamedate asc, gametime asc";
$result = $mysqli->query($query) or die("query error => ".$mysqli->error);
while($rs = $result->fetch_object()){
	$rsc9[]=$rs;
}

$query="select * from xc_games where gamedate>='".$today."' and (hometeam='브렌트퍼드' or awayteam='브렌트퍼드' or hometeam='브렌트퍼드 (Eng)' or awayteam='브렌트퍼드 (Eng)') order by gamedate asc, gametime asc";
$result = $mysqli->query($query) or die("query error => ".$mysqli->error);
while($rs = $result->fetch_object()){
	$rsc10[]=$rs;
}

$query="select * from xc_games where gamedate>='".$today."' and (hometeam='마인츠' or awayteam='마인츠' or hometeam='마인츠 (Ger)' or awayteam='마인츠 (Ger)') order by gamedate asc, gametime asc";
$result = $mysqli->query($query) or die("query error => ".$mysqli->error);
while($rs = $result->fetch_object()){
	$rsc11[]=$rs;
}

$query="select * from xc_games where gamedate>='".$today."' and (hometeam='VfB 슈투트가르트' or awayteam='VfB 슈투트가르트' or hometeam='VfB 슈투트가르트 (Ger)' or awayteam='VfB 슈투트가르트 (Ger)') order by gamedate asc, gametime asc";
$result = $mysqli->query($query) or die("query error => ".$mysqli->error);
while($rs = $result->fetch_object()){
	$rsc12[]=$rs;
}
...
<?php 
	foreach($rsp as $p){
?>
		<div style="width:165px;" class="col-lg-2">
			<div class="card mb-4">
					...
					<?php
						foreach($rsc1 as $r1){
							if($r1->gamedate==$p->gamedate){
								echo "손흥민(".substr($r1->gametime,0,5).")<br>";
							}
						}
					?>
					<?php
						foreach($rsc2 as $r2){
							if($r2->gamedate==$p->gamedate){
								echo "황희찬(".substr($r2->gametime,0,5).")<br>";
							}
						}
					?>
						<?php
						foreach($rsc3 as $r3){
							if($r3->gamedate==$p->gamedate){
								echo "김민재(".substr($r3->gametime,0,5).")<br>";
							}
						}
					?>
					<?php
						foreach($rsc4 as $r4){
							if($r4->gamedate==$p->gamedate){
								echo "이강인(".substr($r4->gametime,0,5).")<br>";
							}
						}
					?>
					<?php
						foreach($rsc5 as $r5){
							if($r5->gamedate==$p->gamedate){
								echo "황의조(".substr($r5->gametime,0,5).")<br>";
							}
						}
					?>
					<?php
						foreach($rsc6 as $r6){
							if($r6->gamedate==$p->gamedate){
								echo "조규성(".substr($r6->gametime,0,5).")<br>";
							}
						}
					?>
					<?php
						foreach($rsc7 as $r7){
							if($r7->gamedate==$p->gamedate){
								echo "황인범(".substr($r7->gametime,0,5).")<br>";
							}
						}
					?>
					<?php
						foreach($rsc8 as $r8){
							if($r8->gamedate==$p->gamedate){
								echo "양현준(".substr($r8->gametime,0,5).")<br>";
							}
						}
					?>
					<?php
						foreach($rsc9 as $r9){
							if($r9->gamedate==$p->gamedate){
								echo "홍현석(".substr($r9->gametime,0,5).")<br>";
							}
						}
					?>
					<?php
						foreach($rsc10 as $r10){
							if($r10->gamedate==$p->gamedate){
								echo "박지수(".substr($r10->gametime,0,5).")<br>";
							}
						}
					?>
					<?php
						foreach($rsc11 as $r11){
							if($r11->gamedate==$p->gamedate){
								echo "이재성(".substr($r11->gametime,0,5).")<br>";
							}
						}
					?>
					<?php
						foreach($rsc12 as $r12){
							if($r12->gamedate==$p->gamedate){
								echo "정우영(".substr($r12->gametime,0,5).")<br>";
							}
						}
					?>
